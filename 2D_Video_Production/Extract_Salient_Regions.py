...
def dbscan_clustering(saliency_features, saliency_points, epsilon, min_samples, image_shape):
    ...
    dbscan = DBSCAN(eps=epsilon, min_samples=0)
    ...
    for i, lst in enumerate(clustered_points_1):
        for item in lst:
            if count == 0:
                ...
                if abs(point_new - item[0]) > abs(image_shape[1]/3):
        ...
        if flag == True:
            for j, item in enumerate(clustered_points_1[hold_count]):
                if (image_shape[1]- item[0]) > abs(image_shape[1]/6):
...
def extract_salient_regions(saliency_map,intensity_value,dbscan_distance,resolution):
    ...
    features = [(sublist[0],sublist[1]) for sublist in saliency_features] # hold only x,y to cluster the salient points
