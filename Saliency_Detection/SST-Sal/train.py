...
if __name__ == '__main__':
    ...
    train_data = RGB(path_to_frames_folder, static_videos, load_gt, process=process, frames_per_data=clip_size,
    ...
    validation_data = RGB(path_to_frames_folder, static_val_videos, load_gt, process=process, frames_per_data=clip_size,
                          resolution=resolution)
    validation_loader = DataLoader(validation_data, batch_size=1, shuffle=False, drop_last=True)

    model = train_(train_loader, validation_loader, model, device, criterion, optimizer, EPOCHS=epochs,
